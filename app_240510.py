...
if page == "Teknik Analiz":
    ...
    price_type = st.radio("Hangi fiyat türünü görmek istersiniz?", ["Türk Lirası", "Dolar"])
    ...
    short_window_str = st.sidebar.text_input("Kısa Pencere (5-50)", value="10")  # Metin olarak alıyoruz
    long_window_str = st.sidebar.text_input("Uzun Pencere (60-200)", value="100")  # Metin olarak alıyoruz
    ...
    df = calculate_moving_averages_currency(df, short_window, long_window, 'USD_Close' if price_type == "Dolar" else 'Close')
    ...
    if price_type == "Türk Lirası":
        ...
        ax.plot(buy_signals.index, buy_signals['USD_Close'], '^', markersize=10, color='g', label='Alım Sinyali')
        ax.plot(sell_signals.index, sell_signals['USD_Close'], 'v', markersize=10, color='r', label='Satış Sinyali')
    ax.plot(subset_df['Short_MA'], label=f'{short_window} Günlük Hareketli Ortalama', color='g')
    ax.plot(subset_df['Long_MA'], label=f'{long_window} Günlük Hareketli Ortalama', color='b')
...
#    ax.plot(sell_signals.index, sell_signals['Close'], 'v', markersize=10, color='r', label='Satış Sinyali')
    ...
    if user_input:
        ...
        try:
            ...
            if not df.empty:
                ...
                plt.title(f"{hisse_kodu} için Hareketli Ortalamalar, Alım/Satış Sinyalleri ve Hacim")
                plt.grid(True)
    ...
    for short_window in range(5, 51, 5):
        for long_window in range(60, 201, 10):
            ...
            if min_length > 0:
                if price_type == "Dolar":
                    gain_loss = sell_signals['USD_Close'].values - buy_signals['USD_Close'].values
                else:    
                    gain_loss = sell_signals['Close'].values - buy_signals['Close'].values
    ...
    results_df = pd.DataFrame(results).sort_values(by='Total Gain/Loss', ascending=False)
